...
export class AnalyticsComponent implements OnInit, OnDestroy
{
 ...
 recentTransactionsDataSource: MatTableDataSource<any> = new MatTableDataSource();
    ...
    getallUser(): void {
        this.http.get<any>(`${environment.apiUrl}/dashboard?profit=true&type=monthly`)
          .subscribe((result: any) => {
            // Retrieve data array from the API response.
            const apiData: MonthlyData[] = result?.data?.result?.monthly || [];
    
            // Create a Map where the key is the month (0-11) extracted from the 'label'
            ...
            // Build a complete list for all 12 months (assuming year 2025).
            const completeData: MonthlyData[] = Array.from({ length: 12 }, (_, monthIndex) => {
              if (dataByMonth.has(monthIndex)) {
                ...
                // Create a placeholder for months with no data.
      ...
      updateTableData(): void {
        if (this.isExpanded) {
          // Show all 12 months
          this.sortedData = this.reportinglist.map((data, index) => ({
            transactionId: new Date(data.label).toLocaleString('default', { month: 'long' }),
            date: data.label,
            profit_count: data.profit_count,
            amount: parseFloat(data.total_profit),
            status: data.profit_count === "0" ? 'pending' : 'completed'
          }));
        } else {
          // Show only the first 6 months
          this.sortedData = this.reportinglist.slice(0, 6).map((data, index) => ({
            transactionId: new Date(data.label).toLocaleString('default', { month: 'long' }),
            date: data.label,
            profit_count: data.profit_count,
            amount: parseFloat(data.total_profit),
