...
export class MaintenanceComponent implements OnInit, OnDestroy, AfterViewInit {
    ...
    @ViewChild('mapContainerSide', { static: false }) mapContainerSide!: ElementRef;
mapSide!: google.maps.Map;
markerSide!: google.maps.Marker;
...
loadSideMapForProperty(address: string) {
  if (!address) {
  ...
  geocoder.geocode({ address: address }, (results: any, status: string) => {
    if (status === 'OK' && results[0]) {
      ...
      if (!this.mapContainerSide) {
        console.error("mapContainerSide is not initialized");
        return;
      }

      // Initialize or update the map with map type controls enabled
      if (!this.mapSide) {
        this.mapSide = new google.maps.Map(this.mapContainerSide.nativeElement, {
          center: location,
          zoom: 15,
          mapTypeControl: true,
        ...
        this.mapSide.setCenter(location);
        this.mapSide.setZoom(15);
      }
      ...
      // Initialize and open Street View panorama at the location by default
      const streetView = this.mapSide.getStreetView();
      streetView.setPosition(location);
      streetView.setPov({
        heading: 270,  // Direction the camera faces
        pitch: 0       // Vertical angle
      });
      streetView.setVisible(true);

    } else {
      console.error('Geocode failed for side map:', status);
    }
  });
}
...
updateStatus(status: string, event?: Event): void {
